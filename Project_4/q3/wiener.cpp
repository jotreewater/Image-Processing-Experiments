...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        std::cout << "Usage: ./wiener image.pgm <k>" << std::endl;
        return -1;
    }
    ...
    // initialize array (image is already assumed to have padding)
    ...
    for (int u = 0; u < N; u++) {
        for (int v = 0; v < N; v++) {
            complex<float> h = H(u - n, v - n);
            complex<float> conj = complex<float>(h.real(), -1.0f*h.imag());
            a[u][v] *= conj / (h*conj + K);
        }
    }
