...
float D_0_squared = 5120;
complex<float> H(int u, int v, float width, int order) {
    float D = std::sqrt(u*u + v*v);

    return 1 / (1 + std::pow( (D*width) / (D*D - D_0_squared) , 2*order));
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        std::cout << "Usage: ./band image.pgm <width> <order>" << std::endl;
