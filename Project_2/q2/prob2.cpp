...
int main(int argc, char * argv[]) {
    ...
    ImageType average7_im(image);
    ImageType average15_im(image);
    ImageType gaussian7_im(image);
    ImageType gaussian15_im(image);

    averaging(image, average7_im, 7);
    averaging(image, average15_im, 15);
    apply_filter(image, gaussian7_im, gaussian7);
    apply_filter(image, gaussian15_im, gaussian15);

    std::string filename;

    filename = std::string("averaging7_") + argv[1];
    writeImage(filename.c_str(), average7_im);

    filename = std::string("averaging15_") + argv[1];
    writeImage(filename.c_str(), average15_im);
...
void averaging(ImageType & orig, ImageType & result, int k) {
    ...
    for (int i =  0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            ...
            result.setPixelVal(i, j, (int) average);
        }
    }
}
